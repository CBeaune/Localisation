...
%-------------------------------------------Figure 1--------------------------------------------------------
% Calculate the measured positions of the lines in the absolute frame
% and the lines closest to these measured positions for display. 
...
for i = 1 : nbRes 
    if calcPhase(i) == 2
        j = j+1 ;
        oEstimatedSensor = [ y1(i) ; y2(i) ; 1 ] ;
        
        oEstimatedSensorNormalized=oEstimatedSensor./[xSpacing;ySpacing;1];
        ...
        if oEstimatedSensorNormalized(2) >0.5
         ...
         if oEstimatedSensorNormalized(1)<oEstimatedSensorNormalized(2) %une verticale est plus proche qu'une horizontale 
            oLine  = round( oEstimatedSensor ./ [xSpacing ; 1 ; 1] ) .* [xSpacing ; 1 ; 1] ;
         else 
            oLine  = round( oEstimatedSensor ./ [1 ; ySpacing ; 1] ) .* [1 ; ySpacing ; 1] ;
         end
          
        estLinePos(:,j)   = oEstimatedSensor(1:2) ;
        exactLinePos(:,j) = oLine(1:2)  ;
    end
end
...
% On top of the path, indicate estimated and real line positions.
...
for i = 1 : length(t) 
    ...
    % measurement vector should be a scalar in R0.
    oMeasSensor = [ y1(i) ; y2(i) ; 1 ] ;
    ...
    if oMeasSensorNormalized(2) >0.5
     ...
     if oMeasSensorNormalized(1)<oMeasSensorNormalized(2) %une verticale est plus proche qu'une horizontale 
        ...
        %Measuerment vector
        Y=oMeasSensor(1:2); %we need both coordinates because even if candidate line is vertical, neighbor line could be vertical or horizontal
        ...
        if abs(oMeasSensorNormalized(1)-oMeasSensorNormalized(2))>0.1 %Condition pour que la ligne soit acceptée %ajouté par Charlotte
            dMahaAll(1,j)       = dMaha            ;
        ...
        for neighborIndex = 1 : 4
            ...
            if neighborIndex<=2 % vertical neighbors
                ...
                YhatNeighbor =   oPneighbor ; % That's inv(oTm)*oPneighbor
